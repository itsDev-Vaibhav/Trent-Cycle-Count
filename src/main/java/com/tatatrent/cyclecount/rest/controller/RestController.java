...
public class RestController {
	...
	public ResponseEntity<Message> processCount(@Valid @RequestBody RequestDto request) throws SQLException {
		logger.info("Inside RestController.processCount()");
		return service.storeCycleCount(request) > 0 ? new ResponseEntity<Message>(new Message(HttpStatus.CREATED.value(), "Record inserted."), HttpStatus.CREATED) : new ResponseEntity<Message>(new Message(), HttpStatus.BAD_REQUEST);
	}
